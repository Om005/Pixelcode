...
import { WriteroomFile } from "../features/fileSlicer";
import { GetroomNode } from "../features/fileSlicer";
import { SparklesIcon } from "lucide-react";
import { useNavigate } from "react-router-dom";
import { Copy } from "lucide-react";
import { useMonaco } from "@monaco-editor/react";
import socket from "../lib/socket";
export default function RoomAuth({roomId}) {
...
const Dashboard = ({roomId}) => {
  ...
  const editorRef = useRef(null) // Add editor reference
  const isLocalChange = useRef(false) // Track if change is from current user
  ...
  const handledownload = () => {
    const blob = new Blob([currRoomfile?.content], {
      type: languages[extoname[`${currRoomfile?.language}`]].type,
    });
    const link = document.createElement("a");
    link.href = URL.createObjectURL(blob);
    link.download = `code.${currRoomfile?.language}`;
    link.click();
  };

  const handlerun = async () => {
    if (currRoomfile?.language == "txt") {
    ...
    const options = {
      ...
      data: {
        language: extoname[`${currRoomfile?.language}`],
        stdin: ip,
        files: [
          {
            name: `index.${currRoomfile?.language}`,
            content: currRoomfile?.content,
          },
        ],
      },
    };
    ...
    inputFile.addEventListener("change", (event) => {
      ...
      if (file) {
        ...
        reader.onload = function (e) {
          ...
          dispatch(setCurrRoomfile({ ...currRoomfile, content: code }));
          dispatch(WriteroomFile({ nodeId: currRoomfile._id, content: code }));
        };
  ...
  useEffect(() => {
    const fetchNode = async () => {
      ...
      const nodeId = sessionStorage.getItem("lstroom");
      if (nodeId !== null) {
        const rsp = await dispatch(GetroomNode({ nodeId }));
  ...
  useEffect(() => {
      socket.on("update-writefile", (node)=>{
        const currFile = currRoomfileRef.current;
        if(currFile._id === node._id){
          let savedPosition = null
        if (editorRef.current) {
          savedPosition = editorRef.current.getPosition()
        }
        dispatch(setCurrRoomfile(node))
  ...
  const handleFileDrop = (e) => {
    ...
    if (file) {
      const reader = new FileReader();
      reader.onload = (event) => {
        setip(event.target.result);
        sessionStorage.setItem("ip", event.target.result);
      };
      reader.readAsText(file);
    }
  };
  ...
  const handleFileDrop2 = (e) => {
    ...
    if (file) {
      const reader = new FileReader();
      reader.onload = (event) => {
        dispatch(setCurrRoomfile({ ...currRoomfile, content: event.target.result }));
        dispatch(
          WriteroomFile({ nodeId: currRoomfile._id, content: event.target.result })
        );
      };
      reader.readAsText(file);
    }
      ...
      <div className=" p-2 md:p-3 rounded-tl-2xl border border-neutral-700 bg-neutral-900 flex flex-col gap-2 flex-1 w-full h-full overflow-hidden">
        <div className="flex gap-2 nunito">
          <div className="h-12 w-full rounded-lg  bg-gray100 dar:bg-neutral-800 flex justify-start items-center">
            ...
            <select
              value={extoname[`${currRoomfile?.language}`]}
              disabled={true}
  ...
  className="group relative px-4 py-2.5 bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 text-white rounded-lg font-medium text-sm transition-all duration-300 shadow-lg hover:shadow-xl hover:shadow-blue-500/25 border border-blue-500/20"
  variants={{
    hover: { 
      scale: 1.02,
      y: -1,
...
>
  ...
  {/* Shine effect */}
  <div className="absolute inset-0 rounded-lg bg-gradient-to-r from-transparent via-white/10 to-transparent opacity-0 group-hover:opacity-100 group-hover:animate-pulse transition-opacity duration-300" />
  
  {/* Glow effect */}
  <div className="absolute -inset-0.5 bg-gradient-to-r from-blue-600 to-blue-700 rounded-lg blur opacity-0 group-hover:opacity-30 transition-opacity duration-300 -z-10" />
</motion.button>
          </div>
          ...
          <div className="h-12 w-full rounded-lg  bg-gray100 dak:bg-neutral-800 flex justify-end mr-9 gap-1 items-center">
            ...
            <div className="text-white">
              <button
                onClick={() => {
                  const ok = {
                    arg: currRoomfile?.content,
                    fname: currRoomfile?.name,
                    isrefer: true,
                    type: currRoomfile?.language,
                  };
                  navigate("/chat", { state: ok });
                }}
                className="group p-2 bg-[#3C2882] border border-slate-500 rounded-lg hover:w-40 w-40 mr-2 transition-all duration-200 flex items-center overflow-hidden"
              >
                <SparklesIcon className="h-5 w-5 text-indigo-300 flex-shrink-0" />
                <span className="ml-2 text-indigo-200 montserrat text-sm opacity-1 group-hover:opacity-100 transition-opacity duration-100 whitespace-nowrap">
                  Refer File & Ask
                </span>
              </button>
            </div>
                              ...
                              onClick={() => {
                                ...
                                dispatch(
                                  setCurrRoomfile({
                                    ...
                                    content:
                                      languages[
                                        extoname[`${currRoomfile?.language}`]
                                      ].boilerplate,
                                  })
                                ...
                                socket.emit("file-written", roomId, {...currRoomfile, content: languages[
                                        extoname[`${currRoomfile?.language}`]
                                      ].boilerplate});
                                setshowPopup(false);
                              }}
                ...
                >
                  ...
                  <Editor
                    ...
                    onChange={(e) => {
                      ...
                      socket.emit("file-written", roomId, {...currRoomfile, content: e});
                      dispatch(setCurrRoomfile({ ...currRoomfile, content: e }));
                      dispatch(WriteroomFile({ nodeId: currRoomfile._id, content: e }));
                    }}
                    height="100%"
                    language={extoname[`${currRoomfile?.language}`]}
                    theme="vs-dark"

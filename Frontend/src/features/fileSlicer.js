...
export const addFile = createAsyncThunk(
  ...
  async ({ email, parentId, name, language, content }, { rejectWithValue }) => {
    try {
      ...
      const message =
        err.response?.data?.message || err.message || "Failed to add file";
      return rejectWithValue(message);
    }
  }
);
...
export const renameNode = createAsyncThunk(
  ...
  async ({ email, nodeId, newName }, { rejectWithValue }) => {
    try {
      ...
      const message =
        err.response?.data?.message || err.message || "Failed to rename file";
      return rejectWithValue(message);
    }
  }
);
export const WriteFile = createAsyncThunk(
  ...
  async ({ nodeId, content }, { rejectWithValue }) => {
    try {
      ...
      const message =
        err.response?.data?.message || err.message || "Failed to write file";
      return rejectWithValue(message);
    }
  }
);
export const GetNode = createAsyncThunk(
  ...
  async ({ nodeId }, { rejectWithValue }) => {
    try {
      ...
      const message =
        err.response?.data?.message || err.message || "Failed to retrieve file";
      return rejectWithValue(message);
    }
  }
);
